/* some of this was taken from the PegJS JavaScript syntax examples */

{
  ...
  function extractOptional(optional, index, valueIfMissing = null) {
    return optional ? optional[index] : valueIfMissing;
  }
  ...
  // a += 1      lvalue = identifier(a) operator (+) rvalue = expression(1)
  //
  //  =>  a = a + 1
  // assign:  identifier(a) binop(+, variablevalue(a), expression(1))

function buildCompoundAssignment(lvalue, operator, rvalue) {
    const lAsR  = valueAsRvalue(lvalue)
    return {
      type: "Assignment",
      lvalue,
      rvalue: {
  ...
  function valueAsRvalue(lvalue) {
    if (lvalue.type === `Identifier`)
      return { ...lvalue, type: `VariableValue` }

    if (lvalue.type === `QualifiedLValue`)
      return { ...lvalue, type: `QualifiedRValue` }

    throw new Error(`can't create rvalue from: ${JSON.stringify(lvalue)}`)
  ...
  function mergeLineAttributes(attrs) {
    const labels = []
    const result = {}
    for (const attr of attrs) {
      if (attr._lineLabel) {
        labels.push(attr._lineLabel)
      } else {
        Object.assign(result, attr)
      }
    }
...
Assignment
 ...
 / lvalue:LValue _ operator:AssignmentOperator "=" _ rvalue:Expression
    {
      return buildCompoundAssignment(lvalue, operator, rvalue)
    }
...
LValue
  ...
  / root:BaseLValue qualifiers:Qualifier+
    {
      return buildTree(root, qualifiers, (result, node) => {
        return ast({
           type: "QualifiedLValue",
           left: result,
           right: node
...
Qualifier
  ...
  / "." attr:(AttrName / Identifier)         // attribute reference
    {
      return ast({
        qtype: "attr",
        qvalue: attr,
...
IfExpression
  = if _ "(" _ test:LogicalORExpression _ ")" _
    consequent:ExpressionOrBlock _
    else _ alternate:ExpressionOrBlock
    {
      return ast({
        type:       "IfExpression",
        test:       test,
  ...
  / if _ "(" _ test:LogicalORExpression _ ")" _
    consequent:ExpressionOrBlock
    {
      return ast({
        type:       "IfExpression",
        test:       test,
        consequent: consequent,
        alternate:  ast({ type: "Boolean", value: false }),
      })
    }
  / LogicalORExpression

ExpressionOrBlock
  = "{" _ expr:ExpressionList _ "}"
    { return expr }
  / Expression
...
MultiplicativeExpression
  = head:PowerExpression
    tail:(_ MultiplicativeOperator _ PowerExpression)*
    {
      return buildBinaryExpression(head, tail)
    }
...
UnaryExpression "term"
  = operator:UnaryOperator _ argument:QualifiedExpression
    {
      return ast({
...
QualifiedExpression
  = root:FunctionDefOrPrimary suffices:Qualifier*
    {
      return buildTree(root, suffices, (result, node) => {
        return ast({
           type: "QualifiedLValue",
           left: result,
           right: node
...
FunctionDefinitionExpression
  ...
  / param:Identifier _ "=>" _ body: FunctionBody
    {
      return ast({
        type:   "FunctionExpression",
        params: [ param ],
...
FunctionBody
  ...
  / "{" (_ Expression)* _ {
      const loc = location()
      error("Syntax error inside function body", { start: loc.end, end: loc.end })
    }
  / Expression
...
Color
  = model:ColorModel "(" _  params:ColorComponents _ ")"
    { return ast({ type: "ColorLiteralWithModel", model, params }) }

  / "#" hex:(HexByte HexByte HexByte HexByte?) !HexNibble
...
ColorModel
  = ("oklch"i / "rgb"i / "hsl"i / "hsv"i) "a"i?
    { return text().toLowerCase() }

ColorComponents
  = a:Expression _ "," _ b:Expression _ "," _ c:Expression _ "," _ alpha:Expression
    { return [ a, b, c, alpha ] }
  / a:Expression _ "," _ b:Expression _ "," _ c:Expression
    { return [ a, b, c ] }
...
AnimationSequence
  = head:Animation tail:( _ then __ Animation )+
    {
      const body = [ head ]
      tail.forEach(t => {
        t = t[3]
        t.params.followsPrevious = { type: "Boolean", value: true },
        body.push(t)
      })
      return {
        type: "ExpressionList",
...
Animation
  ...
  / move __ .* _ {
       expected(`"move" takes a shape or shape.cardinal, a place, and optional animation parameters,`)
    }

  / rotate __ what:Expression __ by __ angle:Expression _ about __ about:PositionValue _ params:AnimationParams
    {
      return ast({
        type: "Rotate",
        what,
  ...
  / rotate __ what:Expression __ by __ angle:Expression _ params:AnimationParams
    {
      return ast({
        type: "Rotate",
        what,
  ...
  / rotate __ .* _ {
      expected(`"rotate" takes a shape, an angle, and an optional pivot point`)
    }

  / set __  what:LValue (_ "," _ / __ (to __)?) value:Expression _ params:AnimationParams
    {
      return ast({
        type: "Set",
        what,
  ...
  / set __ .* _ {
       expected(`"set" takes a shape.attribute, a new value, and optional animation parameters,`)
    }

  / draw __ what:Expression _ params:AnimationParams
    {
      return ast({
        type: "Draw",
        what,
  ...
  /*   } */
  /* / wait __ .* _ { */
  /*      expected(`You can wait for: an animation; ...",`) */
  /*   } */


/* Waitable *g
  /* = "animation" (__ "to" __ "finish" __)? _ signedOffset:SignedOffset */
  /*   { */
  /*     return ast({ */
  /*       type: "WaitAnimationDone", */
  /*       signedOffset, */
  /*     }) */
  /*   } */
  /* / "animation" (__ "to" __ "finish" __)? */
  /*   { */
  /*     return ast({ */
  /*       type: "WaitAnimationDone", */
  /*       signedOffset: { */
...
AnimationParam
  = take __ time:Expression _
    { return [ "take", time ] }
  / ease __ ease:String _
    { return [ "ease", ease ] }
  / ease __ ease:$([a-zA-Z][a-zA-Z0-9]*) _
...
ShapeDefaultSetter "shape"
  = shape:ShapeName klass:SEClass "." attr:AttrName _ "=" _ value:Expression
    {
      return {
  ...
  / shape:ShapeName "." attr:AttrName _ "=" _ value:Expression
    {
      return {
        type: `ShapeDefaultSetter`,
        shape,
...
Shape "shape"
  ...
  / Arc __ endings:SELineEndings __ start:FromPosition __ end:ToPosition args:( _ ( SELineDraw / SELineLabel / SECommon / SETurn ))*  {
      // Arc -> from X to Y (arrow endings before from/to)
      const attrs = { ...endings, ...mergeLineAttributes(extractList(args, 1)) }
      attrs._start = start
      attrs._end = end
  ...
  // ---- Polyline: line from A then B then C [close] [radius:N]
  / line:LineOrAbbrev __ start:FromPosition waypoints:( __ ThenToPosition )+ closed:( __ close )?
    args:( _ ( SELineEndings / SELineDraw / SELineShape / SELineLabel / SECommon / SERadii ))* {
      ...
      attrs._start = start
      attrs._waypoints = extractList(waypoints, 1)
      if (closed) attrs._closed = true
      return ast({ type: "Shape", shape: "SPolyline", args: attrs })
    }

  / Line __ endings:SELineEndings __ start:FromPosition waypoints:( __ ThenToPosition )+ closed:( __ close )?
    args:( _ ( SELineDraw / SELineShape / SELineLabel / SECommon / SERadii ))* {
      // Line -> from A then B then C
  ...
  // ---- Polyline: line from A to B then C [then D ...] [close]
  / line:LineOrAbbrev __ start:FromPosition __ first:ToPosition waypoints:( __ ThenToPosition )+ closed:( __ close )?
    args:( _ ( SELineEndings / SELineDraw / SELineShape / SELineLabel / SECommon / SERadii ))* {
      const attrs = { ...line, ...mergeLineAttributes(extractList(args, 1)) }
      attrs._start = start
  ...
  / Line __ endings:SELineEndings __ start:FromPosition __ first:ToPosition waypoints:( __ ThenToPosition )+ closed:( __ close )?
    args:( _ ( SELineDraw / SELineShape / SELineLabel / SECommon / SERadii ))* {
  ...
  // ---- Polyline: line to A then B then C (no from, implicit start)
  / line:LineOrAbbrev __ first:ToPosition waypoints:( __ ThenToPosition )+ closed:( __ close )?
    args:( _ ( SELineEndings / SELineDraw / SELineShape / SELineLabel / SECommon / SERadii ))* {
      const attrs = { ...line, ...mergeLineAttributes(extractList(args, 1)) }
      attrs._waypoints = [first, ...extractList(waypoints, 1)]
      if (closed) attrs._closed = true
  ...
  / Line __ endings:SELineEndings __ first:ToPosition waypoints:( __ ThenToPosition )+ closed:( __ close )?
    args:( _ ( SELineDraw / SELineShape / SELineLabel / SECommon / SERadii ))* {
      // Line -> to A then B then C (arrow endings before to, no from)
      const attrs = { line_path: string("straight"), ...endings, ...mergeLineAttributes(extractList(args, 1)) }
      attrs._waypoints = [first, ...extractList(waypoints, 1)]
      if (closed) attrs._closed = true
      return ast({ type: "Shape", shape: "SPolyline", args: attrs })
  ...
  // ---- Line (two-point)
  / line:LineOrAbbrev __ start:FromPosition __ end:ToPosition args:( _ ( SELineEndings / SELineDraw / SELineShape / SELineLabel / SECommon ))*  {
      const attrs = { ...line, ...mergeLineAttributes(extractList(args, 1)) }
      attrs._start = start
      attrs._end = end
  ...
  / Line __ endings:SELineEndings __ start:FromPosition __ end:ToPosition args:( _ ( SELineDraw / SELineShape / SELineLabel / SECommon ))*  {
      // line -> from X to Y (arrow endings before from/to)
      const attrs = { line_path: string("straight"), ...endings, ...mergeLineAttributes(extractList(args, 1)) }
      attrs._start = start
      attrs._end = end
  ...
  / Face __ direction:CardinalVector {
      return ast({
...
ToPosition
  = "to" __ end:PositionValue { return end }
  / Position
...
// I present "the label argument". The idea is that most shapes can have
// associated text. This is implemented by tacking on a label shape to
// the parent shape. To make the higher level code easier, we construct
// the AST for that label here...

SELabel
  = label:RichLabel { return { label } }
  / label:String {
      return {
        label: ast({
          type: "Shape",
...
SESize
  ...
  / width __ width:Expression {
      return { width }
    }
  / height __ height:Expression {
      return { height }
    }

SEFill
  = fill __ fill:NonShapeExpression {
      return { fill }
    }
...
SEStrokeAttr
  = stroke __ color:( NonShapeExpression ) {
    return { stroke: color }
  }
  ...
  / linestyle:(solid / dotted / dashed) {
      return { linestyle: { type: "String", value: linestyle}}
    }

SERotation
  = rotation __ rotation:Expression __ about _ _rotationCenter:PositionValue
    {
      return { rotation, _rotationCenter }
    }
  / rotation __ rotation:Expression
    {
      return { rotation }
    }
...
SELineEndings
  ...
  / "~~"
    {
      return {
        line_path: string("smooth")
      }
    }

  / start:SELineEndStart line_path:LinePath end:SELineEndEnd
    {
      return {
        line_start: start,
        line_end:   end,
  ...
  / start:SELineEndStart line_path:LinePath
    {
      return {
        line_start: start,
        line_path
  ...
  /  line_path:LinePath end:SELineEndEnd
    {
      return {
        line_end: end,
        line_path
...
SELineLength
  = length __ length:Expression {
      return { length }
    }

SELineDraw
  = "nodraw" !IdentifierPart
    { return { draw_progress: { type: "Number", value: 0 } } }

SELineShape
  = straight
    { return {} }
  / stepped
    { return { line_path: { type: "String", value: "stepped" }}}
  / smooth
...
ActualNumber
  = DecimalIntegerLiteral DecimalPoint DecimalDigit+ ExponentPart?
...
SystemFont
  = (
    ...
    { throw new Error(`we don't support system fonts right now`); } // return ast({ type: "SystemFont", "name": text()}) }

FontProperties
  = before:( BeforeSizeProperty __)* size:FontSize __ family:FontFamilyNames
    {
      return { ...mergeAttributes(extractList(before, 0)), ...size, ...family }
    }
...
SizeUnit
  = (
      "Q"  / "ch" / "cm" / "em" / "ex" / "in" / "lh" / "mm" / "pc" /
      "pt" / "px" / "rem" / "vh" / "vmax" / "vmin" / "vw"
    ) !IdentifierPart
...
/*       return ast({ */
/*         type: "DirectiveStyle", */
/*         style: extractList(style, 1).join(""), */
/*       }) */
/*     } */
...
// Needed for string continuation
